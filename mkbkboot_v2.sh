#!/usr/bin/env bash
# mkbkboot v2.3 — 体检 → 交互 → 幂等（始终叫 mkbkboot）
# 用法：curl -sSL https://raw.githubusercontent.com/AlexLing6er/mkbkboot/main/mkbkboot.sh | sudo bash

set -Eeuo pipefail
trap 'echo -e "\n❌ 脚本出错！查看 /var/log/mkbkboot.log"; exit 1' ERR
...
if swapon --noheadings | grep -q "$SWAPFILE"; then
  ...
  if [[ -f $SWAPFILE ]]; then
    SIZE=$(stat -c%s "$SWAPFILE"); EXPECT=$((SWAP_GB*1024*1024*1024))
...
if [[ -f "$LIVE_DIR/fullchain.pem" && -f "$LIVE_DIR/privkey.pem" ]]; then
  ...
  echo "❌ 未找到证书文件，请查看 /var/log/letsencrypt/letsencrypt.log"
  exit 1
fi
